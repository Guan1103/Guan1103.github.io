...
module.exports = function(ctx) {
  ...
  function scanAssetDir(post) {
    ...
    function getAssetId(item) {
      return pathFn.join(assetDir, item).substring(baseDirLength).replace(/\\/g, '/');
    }
    ...
    return fs.stat(assetDir).then(function(stats) {
      ...
      if (common.isTmpFile(item) || common.isHiddenFile) {
        return false;
      }

      var id = getAssetId(item);
      return PostAsset.has(id);
    }).map(function(item) {
